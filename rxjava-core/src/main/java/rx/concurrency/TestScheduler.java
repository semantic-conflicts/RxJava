...
public static class TimedAction<T> {
    public long time;
    public Func2<? super Scheduler,? super T,? extends Subscription> action;
    public T state;
    public TestScheduler scheduler;
    public TimedAction(    TestScheduler scheduler,    long time,    Func2<? super Scheduler,? super T,? extends Subscription> action,    T state){
      this.time=time;
      this.action=action;
      this.state=state;
...
public static class CompareActionsByTime implements Comparator<TimedAction<?>> {
  ...
  @Override public <T>Subscription schedule(  T state,  Func2<? super Scheduler,? super T,? extends Subscription> action){
    return schedule(state,action,0,TimeUnit.MILLISECONDS);
  }
  @Override public <T>Subscription schedule(  T state,  Func2<? super Scheduler,? super T,? extends Subscription> action,  long delayTime,  TimeUnit unit){
